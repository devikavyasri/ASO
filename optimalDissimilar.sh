#!/bin/sh
#This script is used to find dissimilar outcome to given outcome
...
#3. Initailly distance =0 for dissimilar
...
#iteratively find dissimilar optimal outcome
while [ $obtainedDistance -lt $distance ]
do
  ...
  #check if distance < given distance then repeat process
  if [ $obtainedDistance -lt $distance ]
  then
    /usr/bin/java ModifyOptimalOutput $numOfAtoms $outcome $newOutputObtained  >>temp/existingOutcomes.txt
  fi

  #check if distance >= given distance then print optimal answer
